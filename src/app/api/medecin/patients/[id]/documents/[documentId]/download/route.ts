// Fichier: app/api/medecin/patients/[id]/documents/[documentId]/download/route.ts
...
export async function GET(
  ...
  try {
    ...
    console.log('📥 Download Document Médecin API - Début', { patientId, documentId, medecinId });
    ...
    // Retourner le fichier directement (avec Content-Disposition: attachment pour forcer le téléchargement)
    return new NextResponse(buffer, {
      headers: {
        'Content-Type': document.typeMime || 'application/octet-stream',
        'Content-Disposition': `attachment; filename="${encodeURIComponent(document.nomFichier || 'document')}"`,
