// ========================================
// FICHIER: /app/patient/messages/page.tsx
// VERSION AVEC FILTRAGE PAR ADMIN + SUPPRESSION
// ========================================
'use client';
...
export default function MessagesPatient() {
  ...
  const fetchAdmins = async () => {
    try {
      ...
      const response = await fetch('/api/patient/admins');
      ...
      if (result.success && result.admins && result.admins.length > 0) {
        ...
        // Sélectionner Vann Cliff par défaut (ou le premier admin recommandé)
        const defaultAdmin = result.admins.find((a: Admin) => 
          a.prenom === 'Vann' && a.nom === 'Cliff'
        ) || result.admins.find((a: Admin) => a.estRecommande) || result.admins[0];
        
        if (defaultAdmin) {
          console.log('👤 Admin par défaut sélectionné:', defaultAdmin.prenom, defaultAdmin.nom);
          setSelectedAdminId(defaultAdmin.id);
        }
      } else {
        console.warn('⚠️ Aucun admin trouvé dans la réponse');
        setAdmins([]);
      }
    } catch (err) {
  ...
  const fetchMessages = async () => {
    try {
      ...
      if (result.success) {
        ...
        result.messages
          .filter((msg: Message) => !msg.estLu && msg.expediteur)
  ...
  const deleteMessage = async (messageId: string) => {
    ...
    try {
      const response = await fetch(`/api/patient/messages?messageId=${messageId}`, {
        method: 'DELETE'
      });
  ...
  const handleQuickAction = (action: string) => {
    ...
    switch (action) {
      case 'rdv':
        template = 'Bonjour, je souhaiterais prendre un rendez-vous. Pourriez-vous me proposer des créneaux disponibles ?';
        break;
      case 'devis':
        template = 'Bonjour, j\'ai une question concernant mon devis. ';
        break;
      case 'probleme':
        ...
        <div className="max-w-5xl mx-auto h-[calc(100vh-8rem)] flex flex-col">
          ...
          <div className="mb-6">
            ...
            <p className="text-gray-600">
              Communiquez avec l'équipe Perfect Assistance
              {currentAdmin && filteredMessages.length > 0 && (
                <span className="ml-2 text-teal-600 font-medium">
                  • {filteredMessages.length} message{filteredMessages.length > 1 ? 's' : ''} avec {currentAdmin.prenom} {currentAdmin.nom}
                </span>
              )}
            </p>
          </div>
          ...
          <div className="flex-1 bg-white rounded-2xl shadow-md overflow-hidden flex flex-col">
            {/* Header conversation avec sélecteur d'admin */}
            <div className="px-6 py-4 bg-gradient-to-r from-teal-500 to-cyan-600 text-white">
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    <h2 className="text-lg font-bold flex items-center">
                      {currentAdmin ? `${currentAdmin.prenom} ${currentAdmin.nom}` : 'Équipe Perfect Assistance'}
                      {currentAdmin?.estRecommande && <span className="ml-2">⭐</span>}
                    </h2>
                    <p className="text-sm text-teal-100">
                      {currentAdmin?.role || 'Coordinateurs et assistants médicaux'}
                    </p>
                  </div>
                </div>
              ...
              {admins.length === 0 && (
                <div className="mt-3 text-sm text-teal-100 bg-white/10 rounded-lg px-3 py-2">
                  ℹ️ Les coordinateurs seront bientôt disponibles
                </div>
              )}
            </div>

            {/* Bandeau de recommandation (toujours visible si Vann Cliff est sélectionné) */}
            {currentAdmin?.estRecommande && !showAdminSelector && (
              <div className="bg-gradient-to-r from-yellow-50 to-amber-50 border-b border-yellow-200 px-6 py-3">
                <div className="flex items-start">
                  <span className="text-2xl mr-3">⭐</span>
                  <div>
                    <p className="text-sm text-yellow-900 leading-relaxed">
                      <strong className="font-bold">Vann Cliff</strong> est votre coordinateur principal recommandé, 
                      spécialisé dans l'accompagnement personnalisé des patients internationaux avec plus de 5 ans d'expérience 
                      en coordination médicale et assistance voyage.
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  {admins.length > 0 ? admins.map(admin => {
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <div className="text-right">
                            ...
                            {admin.id === selectedAdminId && (
                              <span className="text-teal-600 text-xl ml-2">✓</span>
                            )}
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="text-center py-6 text-gray-500">
                      <p>Aucun coordinateur disponible pour le moment</p>
                    </div>
                  )}
                </div>
                
                {admins.some(a => a.estRecommande) && (
                  <div className="mt-3 p-3 bg-yellow-100 rounded-lg">
                    <p className="text-sm text-yellow-800">
                      💡 <strong>Vann Cliff</strong> est notre coordinateur principal recommandé, 
                      spécialisé dans l'accompagnement des patients internationaux.
                    </p>
                  </div>
                )}
              </div>
              ...
              {filteredMessages.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600 mb-6">
                    {currentAdmin 
                      ? `Commencez une conversation avec ${currentAdmin.prenom} ${currentAdmin.nom}`
                      : 'Sélectionnez un coordinateur pour commencer'}
                  </p>
                </div>
              ) : (
                <>
                  {filteredMessages.map(msg => (
                    <div 
                      key={msg.id}
                      className={`flex ${msg.expediteur ? 'justify-start' : 'justify-end'}`}
                    >
                      <div className={`max-w-[70%] ${msg.expediteur ? 'order-2' : 'order-1'}`}>
                        {msg.expediteur && (
                          <div className="flex items-center mb-1 ml-4">
                            <div className="w-8 h-8 bg-gradient-to-br from-teal-500 to-cyan-600 rounded-full flex items-center justify-center mr-2">
                        ...
                        <div className={`rounded-2xl px-4 py-3 ${
                          ...
                          {msg.sujet && (
                            <p className={`text-sm font-bold mb-1 ${
                              msg.expediteur ? 'text-gray-900' : 'text-white'
                            }`}>
                              {msg.sujet}
                            </p>
                          )}
                        ...
                        {/* Afficher l'heure et le bouton supprimer pour les messages du patient */}
                        {!msg.expediteur && (
            ...
            <div className="p-4 bg-white border-t border-gray-200">
              <div className="flex items-center space-x-3 mb-3">
                <button
                  ...
                  className="px-3 py-1.5 text-sm bg-gray-100 hover:bg-teal-100 hover:text-teal-700 text-gray-700 rounded-lg transition-colors flex items-center"
                >
                  📅 Demander un RDV
                </button>
                <button
              ...
              <div className="flex items-center space-x-3">
                ...
                <input
                  ...
                  placeholder={`Message à ${currentAdmin ? `${currentAdmin.prenom} ${currentAdmin.nom}` : 'l\'équipe'}...`}
                  disabled={sending}
                  className="flex-1 px-4 py-3 border border-gray-300 rounded-xl focus:border-teal-500 focus:ring-2 focus:ring-teal-200 outline-none disabled:bg-gray-100 disabled:cursor-not-allowed"
                />
                
                <button
                  onClick={sendMessage}
                  disabled={sending || !newMessage.trim()}
                  className="px-6 py-3 bg-gradient-to-r from-teal-500 to-cyan-600 hover:from-teal-600 hover:to-cyan-700 text-white font-medium rounded-xl transition-all shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
                >
                  {sending ? (
                    <>
