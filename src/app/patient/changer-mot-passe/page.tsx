...
export default function ChangerMotDePassePage() {
  ...
  // ✅ Gérer les deux paramètres : first=true OU obligatoire=true
  const isFirstLogin = searchParams.get('first') === 'true' || searchParams.get('obligatoire') === 'true';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const endpoint = userType === 'medecin' 
        ? '/api/medecin/changer-mot-de-passe'  // ✅ Corrigé
            ...
            <Link href="/" className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-600">
                  {userType === 'medecin' ? 'Espace Médecin' : 'Espace Patient'}
                </p>
              </div>
            </Link>
          </div>
          ...
          <div className="bg-white rounded-3xl shadow-2xl overflow-hidden">
            <div className="bg-gradient-to-r from-[#4DB8A8] to-[#3DA391] px-8 py-8 text-center">
              ...
              <h1 className="text-3xl font-bold text-white mb-2">
                {isFirstLogin ? 'Changement obligatoire' : 'Changer le mot de passe'}
              </h1>
                ...
                <div>
                  ...
                  <div className="mt-2 space-y-1">
                    ...
                    <ul className="text-xs space-y-1 ml-4">
                      <li className={newPassword.length >= 8 ? 'text-green-600 font-medium' : 'text-gray-500'}>
                        {newPassword.length >= 8 ? '✅' : '⭕'} Au moins 8 caractères
                      </li>
                      <li className={/[A-Z]/.test(newPassword) ? 'text-green-600 font-medium' : 'text-gray-500'}>
                        {/[A-Z]/.test(newPassword) ? '✅' : '⭕'} Une lettre majuscule
                      </li>
                      <li className={/[a-z]/.test(newPassword) ? 'text-green-600 font-medium' : 'text-gray-500'}>
                        {/[a-z]/.test(newPassword) ? '✅' : '⭕'} Une lettre minuscule
                      </li>
                      <li className={/[0-9]/.test(newPassword) ? 'text-green-600 font-medium' : 'text-gray-500'}>
